  ...
  describe 'V1::Membership', type: :request do
    ...
    describe 'when create' do
      ...
      it_behaves_like 'an authenticated resource' do
        before { post beach_api_core.v1_memberships_path,
                      params: { membership: { member_id: user.id, member_type: 'User' },
                                group_id: organisation.id, group_type: 'Organisation' } }
      end

      it 'should create membership' do
        expect do
          post beach_api_core.v1_memberships_path,
               params: { membership: { owner: true, member_id: user.id, member_type: 'User' },
                         group_id: organisation.id, group_type: 'Organisation' },
               headers: bearer_auth
