class AddUniqIndexToInteractionAttributes < ActiveRecord::Migration[5.1]
  def change
    BeachApiCore::InteractionAttribute.select('interaction_id,key')
      ...
      interaction_attributes = BeachApiCore::InteractionAttribute.where(interaction_id: interaction_attribute.interaction_id,
                                                                        key: interaction_attribute.key).to_a
      interaction_attributes.pop
      interaction_attributes.each(&:destroy)
    end
