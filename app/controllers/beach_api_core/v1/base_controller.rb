...
module BeachApiCore
  class V1::BaseController < ApplicationController
    ...
    def explicit_application_authorize!
      ...
      render_json_error({ message: t('api.resource_description.errors.unauthorized') }, :unauthorized)
    end

    def application_authorize!
      if !doorkeeper_token && request.headers['HTTP_AUTHORIZATION'].present?
        unless application_authorized?
          return render_json_error({ message: t('api.resource_description.errors.unauthorized') },
                                   :unauthorized)
        end
